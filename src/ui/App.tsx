...
function MuseumFloor({
  museum, buildingName, selectedRoom, onSelectRoom,
}: {
  museum: ReturnType<typeof E.activeMuseum>;
  buildingName: string;
  selectedRoom: number | null;
  onSelectRoom: (roomId: number, hallId: string) => void;
}) {
  const openRooms = museum.rooms.filter(r => r.unlocked);
  const visitors = Math.max(2, Math.min(14, Math.round(museum.fame / 5) + 2));
  return (
    <section className="pixel-museum panel" aria-label="Clickable museum floor">
      <div className="pixel-floor-head">
        <div>
          <h2>Museum Floor<span className="sub">{buildingName}</span></h2>
          <p className="empty-note">Click a gallery to curate it. The rooms below are your museum, not a separate menu.</p>
        </div>
        <div className="open-sign">OPEN</div>
      </div>
      <div className="pixel-building">
        <div className="pixel-entrance"><span>◆</span> Entrance</div>
        <div className="pixel-room-grid">
          {openRooms.map((room, i) => {
            const style = room.theme ? STYLES[room.theme] : null;
            const art = room.items.slice(0, 4).map(id => ARTIFACT_BY_ID[id]).filter(Boolean);
            return (
              <button key={room.id}
                className={'pixel-room' + (selectedRoom === room.id ? ' selected' : '')}
                onClick={() => onSelectRoom(room.id, room.hallId)}>
                <span className="pixel-room-title">{style?.name || 'Empty Gallery'}</span>
                <span className="pixel-wall">
                  {art.length ? art.map(a => (
                    <span key={a.id} className="pixel-frame" title={a.name}>
                      {a.name.split(/\s+/).slice(0,2).map(w => w[0]).join('')}
                    </span>
                  )) : <span className="pixel-empty-wall">Choose a theme</span>}
                </span>
                <span className="pixel-room-foot">Room {room.id + 1} · {room.items.length} works</span>
                {i % 2 === 0 && <span className="pixel-bench" />}
              </button>
            );
          })}
          {museum.rooms.filter(r => !r.unlocked).slice(0, 2).map(room => (
            <div className="pixel-room locked" key={room.id}>
              <span className="pixel-room-title">Closed Space</span>
              <span className="pixel-empty-wall">Expand to unlock</span>
              <span className="pixel-room-foot">Future gallery</span>
            </div>
          ))}
        </div>
        <div className="pixel-people" aria-hidden="true">
          {Array.from({ length: visitors }, (_, i) => (
            <span key={i} className={'pixel-person p' + (i % 6)} style={{ left: `${7 + ((i * 17) % 86)}%`, top: `${12 + ((i * 29) % 72)}%` }}>
              <i />
            </span>
          ))}
        </div>
      ...
      <div className="pixel-statusbar">
        <span>Today’s visitors: {Math.max(3, Math.round(museum.fame * 1.7))}</span>
        <span>{openRooms.length} galleries open</span>
        <span>{museum.rooms.flatMap(r => r.items).length} works displayed</span>
      </div>
    </section>
  );
}
...
function GalleryRoom({
  ...
  // heldArt may be an owned-art id, or "loan:<loanId>" — resolve
  // either to the underlying work's style for the drop-target hint
  let heldStyle: string | null = null;
  if (heldArt) {
    if (heldArt.startsWith('loan:')) {
      const loanId = heldArt.slice(5);
      const loan = E.activeMuseum(state).loans.find(l => l.id === loanId);
      if (loan) heldStyle = ARTIFACT_BY_ID[loan.artifactId].style;
    } else {
